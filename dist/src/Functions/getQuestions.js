 ...
 */
export default async function getQuestions(options) {
    const link = EasyTriviaUtil.links.base.GET_QUESTIONS;
    const defaultOptions = {
        amount: 10,
        encode: QuestionEncodings.none,
    };
    if ((options === null || options === void 0 ? void 0 : options.category) instanceof Category)
        options.category = options.category.id;
    if ((options === null || options === void 0 ? void 0 : options.session) instanceof Session) {
    ...
    const data = (await EasyTriviaUtil.openTDBRequest(finalLink));
    let questions = EasyTriviaUtil.parseRawQuestions(data.results);
    if (targetEncode == "none" && finalOptions.encode == "base64") {
        questions = questions.map((q) => EasyTriviaUtil.base64Decoder.decodeObjectValues(q));
    }
    return questions;
}
